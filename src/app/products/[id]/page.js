...
export default async function ProductPage({ params }) {
  ...
  const res = await fetch(`${backendURL}/add-product/${id}`, { cache: "no-store" });
    ...
    <div className="p-6 max-w-3xl mx-auto bg-white rounded-xl shadow-lg border border-gray-200">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col justify-between">
          <h1 className="text-4xl font-bold mb-4">{product.title}</h1>
          <p className="text-gray-700 mb-4">{product.description}</p>
          <p className="text-2xl font-extrabold text-green-600 mb-4">Price: ${product.price}</p>
          <p className="text-2xl font-extrabold text-blue-600 mb-4">Category: {product.category}</p>
        </div>
      </div>
    </div>
  );
